...
- [x] **Task 12.1 Define canonical example flow (YAML CAPTURE boot + CLI header REPLAY switch)** — example YAML + README canonical flow
...
- [x] **Task 13.5 SoftprobeContext get-or-create cassette per traceId** — `feat(context): get-or-create cassette per (cassetteDirectory, traceId); run() accepts optional storage`
...
- [x] **Task 13.6 Only SoftprobeContext creates cassette instances** — `refactor(cassette): only context creates NdjsonCassette; request-storage and workers use getOrCreateCassette`
...
- [x] **Task 13.11 Restore E2E capture/replay tests broken by Task 13.2** — `E2E restored: init cassetteDirectory from cassettePath; middleware/workers use context cassette; x-softprobe-trace-id + x-softprobe-cassette-path in tests`
  ...
  - **Solution**: Init derives cassetteDirectory from cassettePath when only cassettePath is set. resolveRequestStorage (and context-request-storage) accept cassetteDirectory + traceId for per-request cassette. Express/Fastify middleware pass traceId into resolve. E2E tests send x-softprobe-trace-id (and x-softprobe-cassette-path for REPLAY fixture) so files are at `{cassetteDirectory}/{traceId}.ndjson`. Redis/pg capture workers run inside SoftprobeContext.run with cassetteDirectory + traceId. HTTP capture handler captures cassette/traceId before await so async callbacks keep them. Bypass fetch uses undici.fetch with (url, init). **Note**: task-9-1 (PASSTHROUGH + header CAPTURE) may still return 500 in some envs; fastify-inbound CAPTURE can be flaky in full suite.
